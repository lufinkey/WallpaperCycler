...
void SaxParser::parse()
{
  ...
  std::string error_str = format_xml_parser_error(context_);
...
{
  parse_memory_raw((const unsigned char*)contents.c_str(), contents.length());
}
...
{
  parse_chunk_raw((const unsigned char*)chunk.c_str(), std::strlen(chunk.c_str()));
}
