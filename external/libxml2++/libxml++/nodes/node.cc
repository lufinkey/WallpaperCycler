...
xmlXPathObject* eval_common(const std::string& xpath,
...
bool eval_common_to_boolean(const std::string& xpath,
  const xmlpp::Node::PrefixNsMap* namespaces,
  xmlpp::XPathResultType* result_type, xmlNode* node)
{
...
double eval_common_to_number(const std::string& xpath,
  const xmlpp::Node::PrefixNsMap* namespaces,
  xmlpp::XPathResultType* result_type, xmlNode* node)
{
  xmlXPathObject* xpath_value = eval_common(xpath, namespaces, result_type, node);
...
std::string eval_common_to_string(const std::string& xpath,
  const xmlpp::Node::PrefixNsMap* namespaces,
  xmlpp::XPathResultType* result_type, xmlNode* node)
{
  xmlXPathObject* xpath_value = eval_common(xpath, namespaces, result_type, node);
  ...
  if (result)
  {
    const std::string uresult(reinterpret_cast<const char*>(result));
    xmlFree(result);
    return uresult;
  }
  return std::string();
...
xmlpp::Element* add_child_common(const std::string& name, xmlNode* child, xmlNode* node)
{
  if (!node)
  {
    xmlFreeNode(child);
...
Node* Node::get_first_child(const std::string& name)
{
...
const Node* Node::get_first_child(const std::string& name) const
{
...
Element* Node::add_child_with_new_ns(const std::string& name,
  const std::string& ns_uri, const std::string& ns_prefix)
{
...
_xmlNode* Node::create_new_child_node_with_new_ns(const std::string& name,
  const std::string& ns_uri, const std::string& ns_prefix)
{
  if (impl_->type != XML_ELEMENT_NODE)
    throw internal_error("You can only add child nodes to element nodes.");
...
bool Node::eval_to_boolean(const std::string& xpath, XPathResultType* result_type) const
{
  return eval_common_to_boolean(xpath, 0, result_type, impl_);
}

bool Node::eval_to_boolean(const std::string& xpath, const PrefixNsMap& namespaces,
  XPathResultType* result_type) const
{
  return eval_common_to_boolean(xpath, &namespaces, result_type, impl_);
}

double Node::eval_to_number(const std::string& xpath, XPathResultType* result_type) const
{
  return eval_common_to_number(xpath, 0, result_type, impl_);
}

double Node::eval_to_number(const std::string& xpath, const PrefixNsMap& namespaces,
  XPathResultType* result_type) const
{
  return eval_common_to_number(xpath, &namespaces, result_type, impl_);
}

std::string Node::eval_to_string(const std::string& xpath, XPathResultType* result_type) const
{
  return eval_common_to_string(xpath, 0, result_type, impl_);
}

std::string Node::eval_to_string(const std::string& xpath, const PrefixNsMap& namespaces,
  XPathResultType* result_type) const
{
