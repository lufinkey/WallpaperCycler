...
CdataNode* Element::add_child_cdata(const std::string& content)
{
  xmlNode* child = xmlNewCDataBlock(cobj()->doc, (const xmlChar*)content.c_str(), std::strlen(content.c_str()));
  xmlNode* node = xmlAddChild(cobj(), child);
  if (!node)
  {
    xmlFreeNode(child);
...
EntityReference* Element::add_child_entity_reference(const std::string& name)
{
  const std::string extended_name = name + "  "; // This is at least two chars long.
  int ichar = 0;
